...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /tests/test_infra/test_repo/test_dataset_repo_distributed_partitioned_nlp.py        #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Saturday October 12th 2024 04:07:32 am                                              #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
@pytest.mark.dataset
@pytest.mark.repo
@pytest.mark.repo_dist_part_nlp
class TestDatasetRepoDistributedAdd:  # pragma: no cover
    # ============================================================================================ #
    def test_add_dataset(self, spark_partitioned_nlp_ds, caplog) -> None:
        start = datetime.now()
        logger.info(
    ...
    # ============================================================================================ #
    def test_add_dataset_exists_error(self, spark_partitioned_nlp_ds, caplog) -> None:
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
...
class TestDatasetRepoDistributedGet:  # pragma: no cover
    # ============================================================================================ #
    def test_get_dataset(self, spark_partitioned_nlp_ds, caplog) -> None:
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
...
class TestDatasetRepoDistributedRemove:  # pragma: no cover
    # ============================================================================================ #
    def test_remove_dataset(self, spark_partitioned_nlp_ds, caplog) -> None:
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
