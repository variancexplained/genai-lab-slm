...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /tests/test_infra/test_storage/test_repo/test_zaharia_repo.py                       #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Wednesday September 11th 2024 10:33:38 am                                           #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
single_line = f"\n{100 * '-'}"
# ------------------------------------------------------------------------------------------------ #
INFILEPATH = "notes/ModuleInventory.parquet"
OUTFILEPATH = "data/test/02_dqa/module_inventory.parquet"
...
@pytest.mark.zaharia
@pytest.mark.repo
class TestZahariaRepo:  # pragma: no cover
    # ============================================================================================ #
    ...
    # ============================================================================================ #
    def test_add(self, spark_df, caplog) -> None:
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
        logger.info(double_line)
        # ---------------------------------------------------------------------------------------- #
        repo = ZahariaRepo(file_format=FILEFORMAT)
        repo.add(data=spark_df, stage="02_dqa", name="module_inventory")
