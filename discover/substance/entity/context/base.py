...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /discover/substance/entity/context/base.py                                          #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Friday September 20th 2024 08:11:42 pm                                              #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
from discover.substance.exception.context import InvalidContextException
from discover.substance.value_objects.lifecycle import EPhase
...
class Context(DataClass):
    ...
    -----------
    ephase : EPhase
        The current phase of the pipeline (must be an instance of `EPhase`).
    estage: EStage
    ...
    def _validate(self) -> list:
        ...
        """
        errors = []
        if not isinstance(self.ephase, EPhase):
            errors.append(
                f"Invalid {self.__class__.__name__}. Expected a EPhase instance. Encountered {type(self.ephase).__name__}."
            )
        if not isinstance(self.estage, EStage):
            errors.append(
                f"Invalid {self.__class__.__name__}. Expected a Stage instance. Encountered {type(self.estage).__name__}."
            )
