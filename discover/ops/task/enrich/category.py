...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /discover/ops/task/enrich/category.py                                               #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Thursday December 19th 2024 05:28:57 am                                             #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
from discover.ops.task.enrich.base import EnrichmentTask


# ------------------------------------------------------------------------------------------------ #
class CategoryAggregationTask(EnrichmentTask):
    """
    ...
    def run(self, data: DataFrame) -> DataFrame:
        ...
        Args:
            data (DataFrame): The input PySpark DataFrame with columns:
                ...
                en_review_length, en_review_age, enrichment_tqa1,
                enrichment_tqa2, enrichment_tqa_final.
        ...
        Returns:
            DataFrame: A PySpark DataFrame aggregated at the category level with the following columns:
                ...
                - average_tqa_1: Average TQA score 1 for each category.
                - average_tqa_2: Average TQA score 2 for each category.
                - average_tqa_final: Average final TQA score for each category.
                - max_tqa_1: Maximum TQA score 1 for each category.
                - max_tqa_2: Maximum TQA score 2 for each category.
                - max_tqa_final: Maximum final TQA score for each category.
                - review_highest_vote_sum: Content of the review with the highest vote sum for each category.
                - review_highest_tqa_final: Content of the review with the highest TQA score for each category.
        ...
        window_spec_vote_count = Window.partitionBy("category_id").orderBy(
            F.desc("vote_count")
        )
