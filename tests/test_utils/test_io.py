...
# Version    : 0.1.0                                                                               #
# Python     : 3.12.3                                                                              #
# Filename   : /tests/test_utils/test_io.py                                                        #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
@pytest.mark.io
@pytest.mark.pandasio
class TestPandas:  # pragma: no cover
    # ============================================================================================ #
    ...
    # ============================================================================================ #
    def test_write(self, data_raw, caplog):
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
...
@pytest.mark.io
@pytest.mark.sparkio
class TestPySpark:  # pragma: no cover
    # ============================================================================================ #
    ...
    # ============================================================================================ #
    def test_write(self, spark_dataframe, caplog):
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
        logger.info(double_line)
        # ---------------------------------------------------------------------------------------- #
        filepath = "tests/data/test_pyspark_write.parquet"
        assert isinstance(spark_dataframe, DataFrame)
        logger.info(spark_dataframe.show())
        writer = PySparkWriter()
        writer.write(filepath=filepath, data=spark_dataframe)
        assert os.path.exists(filepath)
